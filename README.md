...
- **52주 고가·저가** 및 현재 위치 시각화 (범위 바) — Yahoo 일별 OHLC 기반(구글/Morningstar와 같은 글로벌 시세), 오늘 실시간값도 구간에 반영
...
> Yahoo의 일별 고가·저가는 장중 극단값을 반영해 구글/Morningstar가 보여주는 52주 범위와 일치합니다(ECB 일일 고시값은 범위가 좁게 나옴). 여기에 **오늘의 실시간값도 구간에 포함**해, 현재값이 범위를 벗어나면 최고/최저와 막대가 즉시 갱신되고 실제 신규 극단값일 때만 "52W 최고/최저" 배지가 표시됩니다. 1년치 재요청은 데이터 절약을 위해 최대 6시간에 1회로 제한합니다.
