...
- **52주 고가·저가** 및 현재 위치 시각화 (범위 바) — 오늘의 실시간값도 구간에 반영되어 최고/최저가 실제로 갱신됨
...
**52주 내역:** frankfurter.dev 시계열 API(ECB 1년치) → 실패 시 localStorage 캐시(24h TTL) 사용

> 과거 1년치를 무료로 제공하는 소스가 frankfurter(ECB)뿐이라 52주 구간은 ECB 기준입니다. 현재 환율(네이버)과 소스가 달라 생기는 불일치를 줄이기 위해, **오늘의 실시간값을 52주 구간에 포함**합니다 — 현재값이 ECB 범위를 벗어나면 최고/최저와 막대가 즉시 갱신되고, 실제 신규 극단값일 때만 "52W 최고/최저" 배지가 표시됩니다.
