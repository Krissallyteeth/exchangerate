...
- **52주 고가·저가** 및 현재 위치 시각화 (범위 바) — MSN Money 기준(구글과 동일한 값), 오늘 실시간값도 구간에 반영. 저점 대비 `+X%`·고점 대비 `−Y%`도 함께 표시
...
> **MSN Money**는 quote 응답에 52주 최고/최저가 들어 있어 **단일 호출(작은 응답)** 로 가져옵니다. 구글이 표시하는 값과 동일합니다 (예: USD/KRW 52주 최저 1,346.80 / 최고 1,561.50). MSN이 막히면 네이버 일별 이력 → Stooq·Yahoo 일별 OHLC → ECB(근사) 순으로 폴백합니다. 여기에 **오늘의 실시간값도 구간에 포함**해, 현재값이 범위를 벗어나면 최고/최저와 막대가 즉시 갱신되고 실제 신규 극단값일 때만 "52W 최고/최저" 배지가 표시됩니다. 재요청은 데이터 절약을 위해 최대 6시간에 1회로 제한하며, 실제 사용된 52주 소스는 푸터에 `52주: MSN / 네이버 일별 / Stooq / Yahoo / ECB 근사`로 표기됩니다.
...
- **52주 내역:** MSN 단일 호출(작은 응답)이며, 최대 6시간에 1회만 재요청 → 데이터 절약 (폴백 시에만 1년치 OHLC 사용)
