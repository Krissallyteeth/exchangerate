...
> Yahoo의 일별 고가·저가는 장중 극단값을 반영해 구글/Morningstar가 보여주는 52주 범위와 일치합니다(ECB 일일 고시값은 범위가 좁게 나옴). 여기에 **오늘의 실시간값도 구간에 포함**해, 현재값이 범위를 벗어나면 최고/최저와 막대가 즉시 갱신되고 실제 신규 극단값일 때만 "52W 최고/최저" 배지가 표시됩니다. 1년치 재요청은 데이터 절약을 위해 최대 6시간에 1회로 제한합니다. 실제 사용된 52주 소스는 푸터에 `52주: 네이버 일별 / Stooq / Yahoo / ECB 근사`로 표기됩니다.

> **참고:** 52주 고가·저가는 데이터 제공사마다 수 원(약 0.3%) 차이가 날 수 있습니다 (예: Yahoo 1,556.39 vs Investing.com 1,561.50). 둘 다 유효한 값이며, Investing.com 등은 무료 공개 API가 없어 그 값을 그대로 가져올 수는 없습니다. 이 앱은 무료·키 불필요·중국 접근 가능한 소스 중 가장 정확한 글로벌 52주 범위(Yahoo)를 사용합니다.
